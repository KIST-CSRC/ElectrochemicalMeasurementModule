...
def print_experiment_data (api, data) :
    ...
    for _ in range(data_info.NbRows) :
        ...
        if tech_name == 'OCV' :
            ...
            # current is a float
            Ec = api.ConvertNumericIntoSingle(row[0])
            
            # current is a float
            I = api.ConvertNumericIntoSingle(row[1])
            
            # Ewe is a float
            Ewe = api.ConvertNumericIntoSingle(row[2])
...
def get_experiment_info (api, data) :
    """ Edited by daeho
...
def get_experiment_data (api,data):
